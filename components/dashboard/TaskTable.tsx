  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3 pl-5 pr-3 sm:pl-6 sm:pr-4">
        <div className="flex justify-between gap-3 w-full items-center md:items-start">
          <h3 className="text-base font-semibold w-fit md:mt-2">
            {title} ({filteredTasks.length})
          </h3>
          {/* desktop search and filters */}
          <div className="hidden flex-1 md:flex flex-col gap-3">
            {/* Search Input */}
            {displaySearch && (
              <div className="relative flex-1 w-full">
                ...
                <InputField
                  ...
                  className={`text-sm pl-8 py-2 w-full ${colors.border}`}
                />
              </div>
            )}
            {/* Proiority Filter */}
            {displayFilters && (
              <div className="flex-1 flex gap-2 sm:gap-3 w-full">
                <SelectField
                  placeholder="Select Priority"
                  value={selectedPriority}
                  onValueChange={setSelectedPriority}
                  containerClassName="sm:gap-2 w-full"
                  selectClassName={`w-full text-sm py-2 ${colors.textMuted} ${colors.border}`}
                  options={[
          ...
          <div className="flex gap-2">
            <Button
              variant={"outline"}
              onClick={() => setDisplaySearch((d) => !d)}
              className={`cursor-pointer px-2 py-2 ${colors.text} ${colors.bgCard} ${colors.border} border text-sm ${colors.hover}`}
            >
              <Search size={16} /> Search
            </Button>
            <Button
              variant={"outline"}
              onClick={() => setDisplayFilters((d) => !d)}
              className={`cursor-pointer px-2 py-2 ${colors.text} ${colors.bgCard} ${colors.border} border text-sm ${colors.hover}`}
            >
        ...
        {/* mobile serach and filters */}
        <div className="md:hidden flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
          {/* Search Input */}
          {displaySearch && (
